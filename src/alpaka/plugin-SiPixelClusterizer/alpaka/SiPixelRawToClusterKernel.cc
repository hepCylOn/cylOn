...
// #include "AlpakaDataFormats/DetId.h"
...
// #include "AlpakaDataFormats/PixelSubdetector.h"
// #include "AlpakaDataFormats/SiPixelDigiConstants.h" -> move from ErrorChecker.h
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  namespace pixelDetails {
    ...
    constexpr int kDetMask = 0xF;
    constexpr int kSubdetMask = 0x7;
    constexpr int kDetOffset = 28;
    constexpr int kSubdetOffset = 25;
    constexpr int PixelBarrel = 1; // PixelSubdetector::PixelBarrel
    
    ALPAKA_FN_ACC bool isBarrel(uint32_t rawId) {
      return (PixelBarrel == ((rawId >> kSubdetOffset) & kSubdetMask));
    }

    ////////////////////

    ALPAKA_FN_ACC uint32_t getLink(uint32_t ww) {
      return ((ww >> ::pixelDetails::LINK_shift) & ::pixelDetails::LINK_mask);
    }

    ALPAKA_FN_ACC uint32_t getRoc(uint32_t ww) {
      return ((ww >> ::pixelDetails::ROC_shift) & ::pixelDetails::ROC_mask);
    }

    ALPAKA_FN_ACC uint32_t getADC(uint32_t ww) {
      return ((ww >> ::pixelDetails::ADC_shift) & ::pixelDetails::ADC_mask);
    }
    ...
    struct RawToDigi_kernel {
      ALPAKA_FN_ACC void operator()(Acc1D const &acc,
        ...
        for (auto gIndex : cms::alpakatools::uniform_elements(acc, wordCounter)) {
          ...
          err[gIndex].pixelErrors() = PixelErrorCompact{0, 0, 0, 0};
          ...
          if (includeErrors and skipROC) {
            ...
            {
              err[gIndex].pixelErrors() = PixelErrorCompact{rawId, ww, errorType, fedId};
          ...
          if (roc > ::pixelDetails::MAX_ROC or link > ::pixelDetails::MAX_LINK) {
            ...
            if (roc > ::pixelDetails::MAX_ROC and roc < 25) {
              ...
              err[gIndex].pixelErrors() = PixelErrorCompact{rawId, ww, error, fedId};
              alpaka::atomicInc(acc, &err.size(), 0xffffffff, alpaka::hierarchy::Blocks{});
            }
            continue;
          }
          ...
          if (layer == 1) {
            ...
            // uint32_t col = ::pixelDetails::getCol(ww);
            // uint32_t row = ::pixelDetails::getRow(ww);
            ...
            // uint32_t dcol = ::pixelDetails::getDCol(ww);
            // uint32_t pxid = ::pixelDetails::getPxId(ww);
    ...
    void SiPixelRawToClusterKernel<TrackerTraits>::makePhase1ClustersAsync(
...

      {
        ...
        alpaka::exec<Acc1D>(queue,
                            ...
                            clusters_d->view().clusInModule().data(),
                            clusters_d->view().clusModuleStart().data() + 1,
