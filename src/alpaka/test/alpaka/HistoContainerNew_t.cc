...
int go(const DevHost& host, const Device& device, Queue& queue) {
  ...
  using HistR = HistoContainer<T, 128, cms::alpakatools::kDynamicSize, 8 * sizeof(T), uint32_t, nParts>;
  ...
  for (int it = 0; it < 5; ++it) {
    ...
    hrv_d.offSize = cms::alpakatools::kDynamicSize;
    ...
    hrv.assoc = hr.data();
    hrv.offSize = cms::alpakatools::kDynamicSize;
...
TEST_CASE("Standard checks of " ALPAKA_TYPE_ALIAS_NAME(alpakaTestHistoContainer), s_tag) {
  SECTION("HistoContainerKernel") {
    ...
    if (devices.empty()) {
      FAIL("No devices available for the " STRINGIFY(ALPAKA_ACCELERATOR_NAMESPACE) " backend, "
