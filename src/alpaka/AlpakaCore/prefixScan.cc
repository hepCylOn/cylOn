...
namespace cms::alpakatools {
  void throwSharedMemoryLimitExceeded(const size_t nElements,
    ...
    std::cout << "Shared memory limit exceeded for prefix scan of " << nElements << " elements in " << nBlocks
  ...
  void throwIterativePrefixScanMaxLevelsExceeded(const size_t nElements, const uint32_t nLevels) {
    std::cout << "Requested an iterative prefix scan for " << nElements << " elements.\n"
