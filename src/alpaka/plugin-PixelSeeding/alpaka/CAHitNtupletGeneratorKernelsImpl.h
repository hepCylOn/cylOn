#ifndef plugin_PixelSeeding_alpaka_CAHitNtupletGeneratorKernelsImpl_h
#define plugin_PixelSeeding_alpaka_CAHitNtupletGeneratorKernelsImpl_h
  ...
  struct kernel_checkOverflows {
    ...
    ALPAKA_FN_ACC void operator()(const TAcc &acc,
      ...
      cms::alpakatools::for_each_element_in_grid_strided(acc, ntNbins, [&](uint32_t idx) {
        ...
        ALPAKA_ASSERT_ACC(foundNtuplets->size(idx) < 6);
        for (auto ih = foundNtuplets->begin(idx); ih != foundNtuplets->end(idx); ++ih)
          ALPAKA_ASSERT_ACC(*ih < nHits);
      });
#endif
...
#endif  // plugin_PixelSeeding_alpaka_CAHitNtupletGeneratorKernelsImpl_h
