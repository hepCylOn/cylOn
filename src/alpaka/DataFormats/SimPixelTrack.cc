...
//   globalPositionVector_.push_back(recHit.globalPosition() - beamSpotPosition_);
  globalPositionVector_.push_back({recHits.xg(recHitId) - beamSpotPosition_.x,recHits.yg(recHitId) - beamSpotPosition_.y,recHits.zg(recHitId) - beamSpotPosition_.z});
...
void SimPixelTrack::sortRecHits(uint32_t const particleId,
                                ParticleSimpleSoA const& particles) {
//   auto vertex = trackingParticleRef_->vertex();
//   sortRecHits(vertex.x(), vertex.y(), vertex.z());
  sortRecHits(particles.vx(particleId), particles.vy(particleId), particles.vz(particleId));
}
...
void SimPixelTrack::sortRecHits(float const x, float const y, float const z) {
  ...
  for (const auto& globalPosition : globalPositionVector_) {
    ...
    std::vector<double> relativePosition = {globalPosition[0] - vertex[0],globalPosition[1] - vertex[1],globalPosition[2] - vertex[2]};
    recHitMag2.push_back((relativePosition[0]*relativePosition[0])+(relativePosition[1]*relativePosition[1])+(relativePosition[2]*relativePosition[2]));
  }
...
//   std::vector<int> sorted_moduleIdVector;
  std::vector<std::vector<double>> sorted_globalPositionVector;
...
void SimPixelTrack::buildSimDoublets() const {
  ...
  for (size_t i = 0; i < layerIdVector_.size(); i++) {
    ...
    for (size_t j = outerLayerStart; j < layerIdVector_.size(); j++) {
      ...
      doublets_.emplace_back(SimPixelTrack::Doublet(*this, i, j, innerDoubletsOfRecHit.at(i)));
...
void SimPixelTrack::buildSimNtuplets(SimPixelTrack::Doublet const& doublet,
  ...
  for (auto const& neighbor : doublet.innerNeighborsView()) {
    ...
    if (startingPairs.find(neighborDoublet.layerPairId()) == startingPairs.end()) {
