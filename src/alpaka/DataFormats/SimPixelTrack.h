 ...
 *
 * SimPixelTracks hold references to all pixel RecHits of a simulated SimParticle.
 ...
 * SimParticle left in the detector. These SimPixelTrack::Doublet objects can be used  
...
public:
  ...
  public:
    ...
    // constructor
    Doublet(SimPixelTrack const&, size_t const, size_t const, std::vector<size_t> const&);
    ...
    std::vector<double> innerGlobalPos() const { return globalPositions_.first; };
    std::vector<double> outerGlobalPos() const { return globalPositions_.second; };
  ...
  private:
    ...
    std::pair<std::vector<double>, std::vector<double>> globalPositions_;  // global position of the RecHits of the Doublet
  ...
  // constructor
  SimPixelTrack(uint32_t const simParticle, BeamSpotPOD const& beamSpot)
      : simParticle_(simParticle), beamSpotPosition_({beamSpot.x, beamSpot.y, beamSpot.z}) {}
...
//       : trackRef_(trackRef), beamSpotPosition_(beamSpot.x0(), beamSpot.y0(), beamSpot.z0()) {}
  SimPixelTrack(BeamSpotPOD const& beamSpot) : beamSpotPosition_({beamSpot.x, beamSpot.y, beamSpot.z}) {}

  // method to add a RecHit to the SimPixelTrack
  void addRecHit(uint32_t const& recHitId,
                 TrackingRecHitSimpleSoA const& recHits,
                 uint8_t const layerId);//,
                //  int16_t const clusterYSize,
                //  unsigned int const detId,
                //  int const moduleId);

  // method to get the reference to the SimParticle
  uint32_t simParticle() const { return simParticle_; }
...
//   int moduleIds(size_t const i) const { return moduleIdVector_[i]; }
  ...
  std::vector<std::vector<double>> globalPositions() const { return globalPositionVector_; }
  // method to get the global position of the RecHit at index i
  std::vector<double> globalPositions(size_t const i) const { return globalPositionVector_[i]; }
...
//   int16_t clusterYSizes(size_t const i) const { return clusterYSizeVector_[i]; }
  ...
  BeamSpotPOD beamSpotPosition() const { return beamSpotPosition_; }
  ...
  std::vector<Doublet>& buildAndGetSimDoublets() const {
    buildSimDoublets();
    return doublets_;
  }
...
private:
  ...
  // class members
  uint32_t simParticle_;  // reference to the SimParticle (if SimPixelTrack is based on a TP)
...
//   std::vector<int> moduleIdVector_;          // vector of the module Ids of the RecHits
  std::vector<std::vector<double>> globalPositionVector_;  // vector of the global positions of the RecHits
                                                   // (corrected by beamspot)
...
//   std::vector<int16_t> clusterYSizeVector_;        // vector of cluster sizes (local y) corresponding to the RecHits
  BeamSpotPOD beamSpotPosition_;  // global position of the beam spot (needed to correct the global RecHit position)
  bool recHitsAreSorted_{false};   // true if RecHits were sorted
  int numLayers_{0};               // number of layers hit by the SimParticle
